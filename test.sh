...
MUST_MATCH="0.0.4 1.2.3 10.20.30 1.1.2-prerelease+meta \
...
MUST_NOT_MATCH="1 1.2 1.2.3-0123 1.2.3-0123.0123 1.1.2+.123 \
...
run_test() {
    ...
    echo ">> Testing valid version numbers: $target <<"
    for var in $MUST_MATCH; do
        if $target "$var"; then
            _ok "$var -> ($major) ($minor) ($patch) ($prerelease) ($buildmetadata)"
        else
